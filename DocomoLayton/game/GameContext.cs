...
using DocomoLayton.game.Storage;
...
public class GameContext : Canvas, ISystemMessageStorage
{
    ...
    {
        FrameCompleted?.Invoke(this, new FrameCompleteEventArgs(isFinal));
    }
    ...
    private IScene a1(IScene paramx1, IScene paramx2)
    {
        if (paramx1 != null)
        {
            ...
            {
                DocomoCsJavaBridge.System.Out.Fatal(throwable, "解放処理で例外発生");
            }
            //System.gc();
        }
        paramx2.Setup(this);
    ...
    public int MoveCursor(int elementRankX, int elementRankY, int[] elementRanksX, int[] elementRanksY, bool paramBoolean)
    {
        ...
        DocomoCsJavaBridge.System.Out.Debug("移動先：");
        int i = rankXOffset;
        int j = rankYOffset;
