...
public class GameFileManager : FileManagerBase
{
    ...
    private bool TryLoadFiles(JavaString[] loadFileNames)
    {
        ...
        for (int var9 = 0; var9 < loadedFileNames.Length; ++var9)
        {
            ...
            DocomoCsJavaBridge.System.Out.Debug("----　読み込むファイルの情報をチェックする ----");
            ...
            if (lastIndex == -1)
            {
                DocomoCsJavaBridge.System.Out.Debug("全ｶﾃｺﾞﾘを検索しましたがデータが存在しません　 \n:{0}\nカテゴリの更新か管理者にデータの確認。\nリリースされるときには出てはいけないエラーになります。", loadedFileNames[var9]);
                return false;
            }
            ...
            if (loadedFileNames[var9].IndexOf(".gif") == -1 && loadedFileNames[var9].IndexOf(".jpg") == -1)
            {
                if (loadedFileNames[var9].IndexOf(".mld") != -1)
                {
                    ...
                    {
                        DocomoCsJavaBridge.System.Out.Fatal(var14, "sound {0}", var14.Message);
                        allFilesLoaded = false;
                        baseDirectoryCount = lastBucket;
            ...
            else
            {
                ...
                {
                    DocomoCsJavaBridge.System.Out.Fatal(var15, "img {0}", var15.Message);
                    allFilesLoaded = false;
                    baseDirectoryCount = lastBucket;
                    return allFilesLoaded;
                }
    ...
    private static byte[] ReadFileData(JavaString filePath)
    {
        ...
        {
            DocomoCsJavaBridge.System.Out.Fatal(exception, "ストレージ読込失敗 sd_storage_name[{0}]データの確認をお願いします {1}", filePath, exception.Message);
        }
